...
1. **Draft your Proposal:** Copy `.github/ISSUE_TEMPLATE/rfc-feature-proposal.md`, populate all fields completely, and place your file in the `/RFCs/` folder named `rfc-[id]-[feature-name].md`.
...
Before submitting a Pull Request, contributors MUST run our unified automated test wrapper from the tools directory:

```bash
# Execute local environment audits, AES-256 decryption tests, and schema compliance checks
./tools/verify-all.sh --self-test
```
