...
class PublisherResigningPipeline {
  ...
  constructor(masterSecret) {
    if (!masterSecret || typeof masterSecret !== "string") {
      throw new Error(
        "Invalid Configuration: Retailer Master Secret must be a secure string.",
      );
    }
  ...
  deriveZksKey(sku, saltHex) {
    ...
    if (!saltHex || !/^[0-9a-fA-F]{32}$/.test(saltHex)) {
      throw new Error(
        "Validation Error: Key Salt must be a 32-character hexadecimal string.",
      );
    }
   ...
   * @param {Object} params.geometry - Wall, portal, zone, and overhead height geometry vectors.
  ...
  async resignAndCompile({
    ...
    zip.addFile(
      "maps/ground_floor/geometry.json",
      Buffer.from(JSON.stringify(geometry, null, 2)),
    );
    zip.addFile(
      "maps/ground_floor/entities.json",
      Buffer.from(JSON.stringify(entities, null, 2)),
    );
    ...
    const mockBasemapBuffer = Buffer.from(
      highResMap.toString("base64").substring(0, 100) +
        "_BAS_MOP_PROX_FALLBACK_WATERMARKED_50PX_PER_GRID",
    );
