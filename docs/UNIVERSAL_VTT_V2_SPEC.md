...
A standard `.uvtt2z` archive must contain the following flat directory structure:
...
Defines the CAD vector layers. All coordinates are mapped in raw pixel space relative to the top-left origin (0,0).
...
_Note: Valid portal states are `open`, `closed`, `locked`, and `broken`._

### 3.3 entities.json

Defines interactive points of interest.
...
_Note: Every object across all JSON files supports the universal `visibility` override flag (`visible`, `gm_only`, `hidden`)._
