...
The **Universal Virtual Tabletop v2 (UVTT v2)** standard replaces legacy flat formats with a performant, topologically aware, and highly optimized campaign packaging framework.

For developers of map-making applications (such as Dungeondraft, Inkarnate, Dungeon Alchemist, or Campaign Cartographer), implementing a high-fidelity v2 export pipeline is critical. The quality of your exporter directly dictates runtime performance inside virtual tabletop (VTT) engines.

This blueprint defines the exact mathematical, algorithmic, and packaging standards required to compile, simplify, sign, and build conforming standard `.uvtt2z` campaign archives, as well as the new Dual-File DRM packages (`.uvtt2z` payload + `.uvtt2k` physical key).

---

### 🎨 1. The Geometry Compiler Pipeline

To optimize memory usage and offload complex curve drawing to GPU vector processors inside VTT viewports, UVTT v2 deprecates raw vertex lists. All wall and portal structures MUST be serialized as native, lightweight **W3C SVG-style path arrays**.
...
Every vector drawn possesses an inherent heading based on the path array indexing sequence. UVTT v2 utilizes this heading to compile directional blockages (such as ledges, one-way windows, or mirror surfaces) using clockwise normal projections:

Given a vector drawn from coordinates $P_1(x_1, y_1)$ to $P_2(x_2, y_2)$:

$$\Delta x = x_2 - x_1, \quad \Delta y = y_2 - y_1$$

The clockwise, Right-Hand normal projection ($\vec{n}_{\text{right}}$) and left-hand normal projection ($\vec{n}_{\text{left}}$) calculate as:
...
GMs and cartographers frequently place wall vectors by hand. Microscopic coordinate offsets at wall junctions (e.g. $x_1 = 12.00$ vs $x_2 = 12.03$) will create single-pixel cracks, causing VTT raycasting engines to leak dynamic light through solid dungeon corners.
...
Segmented drawing (such as drawing four individual straight line vectors to construct a simple hallway wall) creates redundant move commands and floating index keys, wasting GPU computation.

**The Simplification Algorithm:**
Your exporter must traverse adjacent line coordinates and merge colinear segments. For three sequential points ($P_0, P_1, P_2$), calculate the slopes ($m_1$ and $m_2$):

$$m_1 = \frac{y_1 - y_0}{x_1 - x_0}, \quad m_2 = \frac{y_2 - y_1}{x_2 - x_1}$$

If $\vert{}m_1 - m_2\vert{} \le \epsilon$ (where $\epsilon = 10^{-5}$ represents a rounding buffer), the points are collinear. Delete $P_1$ and compile a single cohesive path segment from $P_0$ straight to $P_2$, decreasing vector payload overhead drastically.

---

### 📁 3. Modular Compound ZIP Packaging Pipeline

To resolve memory constraints on lower-spec tablets or browser viewports, UVTT v2 mandates a highly efficient, localized subdirectory package topology. Exporters must discard monolithic centralized `/assets/` schemes in favor of isolated sub-folders.

#### A. Compound Package Structure

All resources for individual levels inside a multi-floor campaign ZIP (`.uvtt2z`) must be grouped within their own slugified maps directory:
...
When compiling a Compound Archive, the exporter must scan all teleport triggered endpoints inside `entities.json`. Any relative map links (`relative://`) pointing to sibling files in the workspace must be rewritten as internal local campaign endpoints (`internal://`) pointing directly to their target sub-directory slug:

- _Before Rewrite:_ `"uri": "relative://tavern_cellar.uvtt2z#lz_ladder_staircase"`
- _After Compound Rewrite:_ `"uri": "internal://tavern-cellar#lz_ladder_staircase"`

---

### 🛡️ 4. Cryptographic Asset Signing & DRM Architecture

To protect your software platform and premium creators from unauthorized asset modification, code injection, or malicious vector scripts inside the ZIP, exporters must generate a root validation receipt. Furthermore, exporters supporting premium storefronts must implement the Dual-File DRM standard.
