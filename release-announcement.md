...
- **Semantic Zones:** Draw structural polygons across your map to represent environmental hazards. By tagging them with agnostic string traits (like `difficult_terrain` or `hazardous`), you allow next-gen VTT platforms to dynamically intercept token movement and pass those boundaries directly into isolated WebAssembly engines, calculating custom movement penalties in real-time.
