...
(.uvtt2z) or fully encrypted GCM envelope (.uvtt2k) format.
"""
...
DEFAULT_SECRET = "RETAILER_MASTER_CRITICAL_SECRET_2026_07_11"
...
def generate_map_image(grid_size=70, grid_count=10, name="Level", watermark=None) -> Image.Image:
    ...
    try:
        ...
        font = None
    draw.text((grid_size * 3, grid_size * 1.2), f"Three-Story Tavern - {name}", fill=(50, 50, 50), font=font)
...
def create_campaign_archive(output_path: str, drm: bool, secret: str, sku: str, salt: str):
    print(f"[*] Starting UVTT v2 Campaign packaging pipeline. Output target: {output_path}")
    ...
    if drm:
        root_manifest["encryption_handshake"] = {
    ...
    preview_img = generate_map_image(grid_size=grid_px, grid_count=10, name="Preview").resize((512, 512), Image.Resampling.LANCZOS)
    ...
    floors = [
        ...
        {"id": "ground_floor", "name": "The Bustling Common Room", "path": "maps/ground_floor/"},
        {"id": "second_floor", "name": "The Quiet Guest Rooms", "path": "maps/second_floor/"}
    ]
    ...
    for floor in floors:
        ...
        files_to_hash[f"{path}manifest.json"] = json.dumps(sub_manifest, indent=2).encode("utf-8")
                ...
                "walls": [
                    {
                        ...
                        "path": [
                            ...
                            {"type": "line", "x": 2.0 * grid_units, "y": 8.0 * grid_units},
        ...
        files_to_hash[f"{path}geometry.json"] = json.dumps(geometry_data, indent=2).encode("utf-8")
        ...
        files_to_hash[f"{path}entities.json"] = json.dumps(entities_data, indent=2).encode("utf-8")

        # Create localized images (map.webp and basemap.webp)
        map_img = generate_map_image(grid_size=grid_px, grid_count=10, name=floor["id"].upper())
        map_bytes = BytesIO()
        map_img.save(map_bytes, format="WEBP", quality=90)
        files_to_hash[f"{path}map.webp"] = map_bytes.getvalue()

        fallback_img = generate_map_image(grid_size=50, grid_count=10, name=floor["id"].upper() + " FALLBACK", watermark="PREVIEW")
    ...
    # 5. Save output or encrypt as GCM envelope
    if drm:
        print("[*] GCM mode enabled. Encrypting the entire zipped package...")
        if not HAS_CRYPTO:
            print("[!] ERROR: cryptography package is missing. Cannot compile .uvtt2k archive.")
            sys.exit(1)
        ...
        print(f"[+] Cryptographic packaging complete. Encrypted size: {len(final_payload)} bytes.")
    else:
        final_payload = raw_zip_bytes
        print(f"[+] Packaging complete. Size: {len(final_payload)} bytes.")

    with open(output_path, "wb") as out_f:
        out_f.write(final_payload)
    print(f"[+] Done! Archive written successfully to: {output_path}")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Three-Story Tavern Conforming Builder")
    parser.add_argument("-o", "--output", default="tavern_three_story.uvtt2z", help="Output path")
    parser.add_argument("--drm", action="store_true", help="Encrypt as .uvtt2k package")
    parser.add_argument("--secret", default=DEFAULT_SECRET, help="Retailer master secret")
    parser.add_argument("--sku", default=DEFAULT_SKU, help="Product SKU")
    parser.add_argument("--salt", default=DEFAULT_SALT, help="Salt checksum")
    
    args = parser.parse_args()
    create_campaign_archive(args.output, args.drm, args.secret, args.sku, args.salt)
