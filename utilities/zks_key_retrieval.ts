 ...
 *
 * Note: Decrypting the payload with the retrieved key will yield the full .uvtt2z container,
 * including premium image layers and the complete geometry.json schema (which contains the
 * UVTT v2 Semantic Movement Zones).
 */
 ...
 */
export async function fetchZksDecryptionKey(
  params: ZksRequestParams,
): Promise<ZksKeyResponse> {
  ...
  try {
    ...
    const response = await fetch(targetUrl.toString(), {
      ...
      headers: {
        ...
        "X-Client-Platform": "UVTT-v2-Web-Client/1.1",
      },
    });
    ...
    console.log(
      "[+] Handshake successful. Decrypted response payload received.",
    );
    return payload;
  } catch (error) {
